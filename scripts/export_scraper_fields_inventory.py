...
def build_contract_rows(sample: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    meta = {
        ...
        "publication.outbox_status": ("outbox", "string", "outbox payload", "domain/publication_event.py::PublicationMetadata.outbox_status", "common", "diagnostic", "Outbox status captured in payload metadata.", "Payload metadata reflects outbox creation time, not later publish updates."),
        ...
        "publication.publisher_service": ("outbox", "string", "outbox payload", "domain/publication_event.py::PublicationMetadata.publisher_service", "common", "diagnostic", "Publisher service name if populated.", "Often null in payload_json because payload is built before publish succeeds."),
        ...
        "publication.published_at": ("outbox", "datetime", "outbox payload", "domain/publication_event.py::PublicationMetadata.published_at", "common", "diagnostic", "Timestamp when publisher marked message as published, if populated.", "Usually null inside stored payload_json."),
    }
    required_publish = {
