...
| `publication.published_at` | `string(date-time) \| null` | нет | В runtime-сообщении обычно `null`, не использовать для бизнес-решений |

Важно:

- `publication.queue_name` может быть `scraper.products.v1`, даже если CRM читает свою очередь `crm.products.import.v1`;
- текущий runtime публикует событие с `publication.outbox_status = "publishing"` и `publication.published_at = null`;
