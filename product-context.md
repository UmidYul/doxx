...
DO NOT:
- change architecture based on this file
- invent new features outside MVP
- override project-context.md
...
2. project-context.md (ARCHITECTURE + DATA)
...
Ключевой инженерный принцип: “DB + shared types = контракт”, чтобы Cursor‑кодер/команда не изобретали разные версии сущностей в разных сервисах. Prisma Migrate предназначен для синхронизации схемы с БД и ведёт историю SQL‑миграций, а `prisma migrate deploy` рекомендуется включать в CI/CD для stage/prod. citeturn6search12turn6search1turn6search5
...
|                        |                               | Routes из pages/                                            |                                              |
|                        |                               | citeturn3search0turn3search12                              |                                              |
+------------------------+-------------------------------+-------------------------------------------------------------+----------------------------------------------+
...
|                        |                               | citeturn3search3turn19search0turn19search4              |                                              |
+------------------------+-------------------------------+-------------------------------------------------------------+----------------------------------------------+
...
|                        |                               | (v5.16+)                                                     | (не указано)                                 |
|                        |                               | citeturn4search0turn0search2turn0search12turn4search1  |                                              |
+------------------------+-------------------------------+-------------------------------------------------------------+----------------------------------------------+
| База данных            | PostgreSQL                     | FTS: tsvector/tsquery + GIN/GiST; jsonb индексация;          | Партиционирование snapshot-таблиц            |
|                        |                               | pg_trgm для fuzzy                                            | citeturn5search3                          |
|                        |                               | citeturn5search0turn5search6turn5search1              |                                              |
+------------------------+-------------------------------+-------------------------------------------------------------+----------------------------------------------+
...
|                        |                               | citeturn6search0turn6search12turn6search1              |                                              |
+------------------------+-------------------------------+-------------------------------------------------------------+----------------------------------------------+
...
|                        |                               | citeturn7search16turn7search1turn7search4turn7search0  |                                              |
+------------------------+-------------------------------+-------------------------------------------------------------+----------------------------------------------+
...
|                        |                               | citeturn8search0turn8search3turn8search2turn8search6  |                                              |
+------------------------+-------------------------------+-------------------------------------------------------------+----------------------------------------------+
...
|                        |                               | и cron jobs                                                   | указано)                                     |
|                        |                               | citeturn9search1turn9search0turn9search2turn9search3  |                                              |
+------------------------+-------------------------------+-------------------------------------------------------------+----------------------------------------------+
...
Почему `sitemap.ts` и Route Handlers именно так: Next.js документирует `sitemap.(js|ts)` как “special file” для генерации sitemap, а Route Handlers — как специальный файл `route.js|ts`, поддерживающий стандартные HTTP‑методы в App Router. citeturn3search2turn3search12turn18search17

Почему отдельный сервис `services/ingest`: BullMQ позиционируется как очередь с воркерами для выполнения фоновых задач, а job schedulers (v5.16+) предназначены для repeatable задач (по cron/interval), что идеально ложится на регулярное обновление цен и обход источников. citeturn4search9turn4search0turn4search12
...
Ключ по поиску: `product.search_vector` (tsvector) + GIN индекс. Postgres документирует GIN/GiST как предпочтительные типы индексов для ускорения полнотекстового поиска, и приводит пример `CREATE INDEX ... USING GIN(tsvector_column)`. citeturn5search0turn5search15

Ключ по фильтрам: `product.attributes` (jsonb) + GIN индекс. Postgres документирует `jsonb` indexing и указывает, что GIN индексы можно использовать для эффективного поиска ключей/пар ключ‑значение в `jsonb`. citeturn5search6turn5search2
...
- gin_product_search(search_vector) USING GIN(search_vector)    # FTS citeturn5search0
- gin_product_attrs(attributes) USING GIN(attributes)          # jsonb citeturn5search6
Optional:
- pg_trgm for fuzzy on title/slug (extension pg_trgm) citeturn5search1
...
Обоснование GTIN: GS1 фиксирует, что EAN/UPC штрихкоды печатаются практически на каждом consumer product, и что GTIN — ключ для идентификации товаров. citeturn11search3turn11search2
...
Scale: партиционирование по captured_at (RANGE) — Postgres документирует декларативное partitioning. citeturn5search3
...
Prisma документирует индексы/уникальные ограничения (`@@index`, `@@unique`) и то, что `prisma migrate deploy` следует запускать в CI/CD для stage/prod. citeturn6search0turn6search1turn6search5
...
Raw SQL миграция (примерно): создать `search_vector` через generated column/trigger и GIN индекс по `tsvector` — Postgres прямо указывает, что `tsvector` колонка подходит для GIN индекса для FTS. citeturn5search0
...
Playwright документирует `page.evaluate()` (выполнение JS в контексте страницы) и network APIs для отслеживания/модификации трафика, включая `page.route()`. citeturn3search3turn19search0turn19search4

### Очереди BullMQ: воркеры, retry/backoff, лимиты и расписание

BullMQ документирует:
- Retries failing jobs с backoff функциями. citeturn0search2
- Rate limiting через опцию `limiter { max, duration }`. citeturn0search12
- Concurrency на воркере (factor > 1) либо несколько процессов. citeturn4search1turn4search5
- Job Schedulers (v5.16+) как замену repeatable jobs и поддержку cron/interval стратегий. citeturn4search0turn4search14
- `UnrecoverableError` для “неисправимых” ошибок, чтобы остановить ретраи. citeturn4search2turn4search6
...
В Postgres это естественно выражается в отдельной таблице `price_snapshot`, а в Scale‑фазе она почти неизбежно растёт и становится кандидатом на partitioning по времени (Postgres документирует декларативное partitioning). citeturn5search3
...
- Уровень 1 (GTIN): если в оффере есть GTIN/EAN/UPC, матчим через `product_gtin`. Сергейность GTIN подтверждается тем, что EAN/UPC печатаются “virtually every consumer product”, и GS1 рассматривает GTIN как ключ идентификации. citeturn11search3turn11search2
...
BullMQ документирует limiter и остановку ретраев через `UnrecoverableError`. citeturn0search12turn4search2turn4search1

## API‑контракт, авторизация и rate limits

### Базовые факты и ограничения реализации (Route Handlers)

Route Handlers в Next.js позволяют определять обработчики HTTP‑методов (`GET`, `POST`, `PUT`, `PATCH`, `DELETE`, `HEAD`, `OPTIONS`) в файле `route.ts|js`. Это удобно для MVP‑BFF API, но требует обязательного rate limiting и RBAC, потому что endpoints публичны. citeturn3search12turn3search0

### Авторизация и роли

Роли (MVP): `USER`, `SELLER`, `ADMIN`. Выбор провайдера аутентификации — **не указано** (может быть любая реализация, совместимая с Next.js). Next.js отдельно даёт guide по реализации auth и рекомендует декомпозировать auth‑процесс. citeturn18search14

Security baseline для API: OWASP подчёркивает риск Broken Object Level Authorization (BOLA) при манипуляции object IDs в path/query/payload, поэтому RBAC/ACL должны проверяться на каждый объект. citeturn10search2turn10search18
...
На периметре WAF: Cloudflare документирует rate limiting rules как способ задавать лимиты и действие при превышении (block/challenge/log). citeturn10search3turn10search7
...
Next.js документирует механизмы кеширования и revalidation, а также отдельный guide “Caching and Revalidating”. citeturn3search1turn3search5turn3search9
...
- Search выдача: dynamic (по умолчанию), но можно кэшировать популярные запросы на сервере через Next caching primitives (реализация — не указано). citeturn3search1turn3search5

### Metadata и sitemap

- `generateMetadata` для динамических title/description/canonical и share‑метаданных — Next.js даёт функцию generateMetadata в API reference. citeturn18search0
- `sitemap.(js|ts)` — специальный файл, кешируется по умолчанию (в зависимости от dynamic APIs), и поддерживает генерацию sitemap. citeturn3search2turn18search17
- Для больших каталогов: `generateSitemaps` генерирует несколько sitemap, которые будут доступны по `/.../sitemap/[id].xml`. citeturn18search1
...
Docker Compose предназначен для определения и запуска multi‑container приложений, а multi‑stage builds помогают отделить build‑зависимости от runtime и уменьшить размер финального образа/attack surface. citeturn20search4turn20search1turn20search5
...
Переменные окружения в Compose: Docker документирует `.env`/`env_file` и правила интерполяции. citeturn20search2turn20search6

Миграции: Prisma рекомендует `migrate deploy` в CI/CD для stage/prod. citeturn6search1turn6search5

### CI/CD: GitHub Actions (пример pipeline и таблица)

GitHub Actions документирует, что workflow — это YAML‑файл, состоящий из jobs, а секреты задаются через secrets context. Также есть официальные гайды по service containers для PostgreSQL и Redis. citeturn8search0turn8search3turn8search2turn8search6
...
+-------------------+------------------------------+----------------------------------------------+
| workflow YAML      | описать jobs/steps в YAML     | workflow syntax citeturn8search0          |
| secrets            | хранить токены/ключи в secrets| using secrets citeturn8search3            |
| service containers | поднять Postgres/Redis в CI    | Postgres/Redis containers citeturn8search2turn8search6 |
| migrations         | prisma migrate deploy          | Prisma deploy guide citeturn6search1turn6search5 |
...
Railway документирует, что variables используются как environment variables при build/run, а также описывает build/deploy lifecycle и environments. citeturn9search1turn9search0turn9search23  
Render документирует background workers (для постоянной обработки очередей) и cron jobs (ограничения длительности run, и рекомендацию использовать worker для длительных задач). citeturn9search2turn9search3
...
OpenTelemetry описывает Collector как vendor‑agnostic компонент для receive/process/export telemetry, а также отдельно описывает signals: traces (span как unit of work) и metrics (measurement captured at runtime). citeturn7search16turn7search2turn7search3
...
OTLP exporter env vars документированы OpenTelemetry как способ конфигурировать endpoint. citeturn7search4
...
Robots policy: RFC9309 прямо указывает, что правила robots — не авторизация, и robots.txt не заменяет реальные меры защиты; перечисление путей в robots.txt делает их discoverable. citeturn10search0
...
DMCA / takedown: если продукт публичный и может получать претензии правообладателей, полезно иметь DMCA‑style notice‑and‑takedown процесс. U.S. Copyright Office указывает, что регистрация авторского права не обязательна для отправки takedown notice (DMCA Section 512). Вне США детали зависят от юрисдикции (правовой контекст Узбекистана — не указан, требуется отдельная юридическая валидация). citeturn10search1turn10search5

PII handling:
- Минимизировать хранение email/phone/ip/user_agent, включить retention и доступ‑контроль (конкретные сроки — не указано).
- Защитить object‑level доступ (BOLA) на каждом endpoint, где есть `:id`. citeturn10search2

WAF/rate limiting:
- На периметре: Cloudflare rate limiting rules (limit + action). citeturn10search3

### Тесты: unit/integration/e2e/contract и fixtures

E2E целесообразно делать на Playwright Test: Playwright документирует retries/timeout и централизованные locators как основу устойчивых тестов. citeturn19search9turn19search3turn19search6
...
MCP задаёт стандарт, по которому серверы могут предоставлять клиенту три класса возможностей: resources, tools, prompts. Документация MCP прямо описывает эти три capability. citeturn17view0  
Tools в MCP имеют имя и schema параметров; MCP спецификация описывает tools и наличие `inputSchema` на базе JSON Schema. citeturn12search13turn0search3  
Transport: MCP определяет stdio и Streamable HTTP; JSON‑RPC сообщения должны быть UTF‑8; клиенты “SHOULD support stdio whenever possible”. citeturn15search18
...
- Примеры конфигураций: remote (url + headers) и local (docker command + args + env) citeturn14view0
...
Важно по секретам: GitHub MCP пример показывает, что `env` может содержать секрет (PAT) и прокидывается в Docker через `-e` и `env` в конфиге. Для команды это означает: `.cursor/mcp.json` обычно не коммитится, а хранится локально; в репо держится только `mcp.example.json` + инструкция, как заполнить (механизм командного управления секретами — не указан). citeturn14view0turn8search3

#### Проблема env vars в MCP и командный workaround

В обсуждениях Cursor community отмечается, что для безопасности Cursor не передаёт все локальные `.env` переменные всем MCP серверам, и рекомендуемый workaround — wrapper script, который загружает env и затем запускает MCP server. citeturn12search18turn12search2
...
Техническая база:
- MCP TypeScript SDK репозиторий описывает, что SDK работает на Node.js/Bun/Deno и включает server libraries (tools/resources/prompts) и transports (Streamable HTTP, stdio). citeturn16view0
- MCP “Build an MCP server” guide подчёркивает важность логирования в stderr для STDIO серверов, чтобы не повредить JSON‑RPC сообщения. citeturn17view0
...
Если хотите, чтобы этот MCP server был “оформленным” по спецификации (tool schemas, JSON Schema 2020‑12, schema discovery), ориентируйтесь на MCP docs: tools имеют `inputSchema`, MCP использует JSON Schema (default 2020‑12), а transports — stdio/Streamable HTTP. citeturn12search13turn0search3turn15search18
...
- Базовая observability: latency/error rate для API + ingest metrics (через OpenTelemetry/Collector). citeturn7search16turn7search2turn7search3